...
if __name__ == '__main__':
    ...
    train_node = torch.from_numpy(np.array(labels[0])[:, 0]).type(torch.cuda.LongTensor)  # Training nodes and labels
    ...
    for l in range(1):
        model = HGRAN(num_edge=A.shape[- 1],
        ...
        if adaptive_lr == 'False':
            ...
            optimizer = torch.optim.Adam([{'params': model.linear1.parameters()},
                                          ...
                                          {'params': model.HomogRelationAttention.parameters(), "lr": 0.01,"weight_decay": 0.001},
                                          {'params': model.HeterogRelationAttention.parameters(), "lr": 0.01,"weight_decay":0.001},
